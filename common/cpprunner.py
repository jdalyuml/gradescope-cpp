...
class CppTest:
    ...
    def _compile(self):
        process = subprocess.run([f'make {self.prog}.test'],
                stdout=subprocess.PIPE, stderr=subprocess.STDOUT, shell=True, text=True, timeout=self.timeout)
        if process.returncode:
            self.msg += '\n'
            self.msg += process.stdout
        return process.returncode == 0

    def _execute(self):
        process = subprocess.run([f'./{self.prog}.test'],
                stdout=subprocess.PIPE, stderr=subprocess.STDOUT, shell=True, text=True, timeout=self.timeout)
        if process.returncode:
            self.msg += '\n'
            self.msg += process.stdout
...
def RunFile(filename, outfile):
    ...
    with open (outfile, 'w') as fout:
        if not tests or not Config.RunImp():
            with open (outfile, 'w') as fout:
                fout.write('')
        else:
